...
const SECTIONS = [
  { id: "upload", label: "1. Peça" },
  { id: "model", label: "2. Modelo" },
  { id: "style", label: "3. Estilo" },
  { id: "generate", label: "4. Gerar" },
] as const;
...
const ProjectPage = () => {
  ...
  useEffect(() => {
    ...
    const variants: ProductVariant[] = (dbVariants || []).map(v => ({
      id: v.id, productId: v.product_id, colorName: v.color_name,
    ...
    setState({
      step: 0, variants, activeVariantId: activeVariant?.id || "",
      ...
      generatedImages: [], weeklyLaunches, activeWeek: weeklyLaunches[0]?.id || "",
    });
    setLoaded(true);
  ...
  const addVariant = async () => {
    ...
    const newVariant: ProductVariant = { id: data.id, productId: data.product_id, colorName: data.color_name, uploadedImages: [], garmentAnalysis: null, sortOrder: data.sort_order };
  ...
  const deleteVariant = async (variantId: string) => {
    ...
    setState(s => {
      ...
      return { ...s, variants: newVariants, activeVariantId: newActive?.id || newVariants[0]?.id || "", uploadedImages: newActive?.uploadedImages || [], garmentAnalysis: newActive?.garmentAnalysis || null, weeklyLaunches: s.weeklyLaunches.filter(w => w.variantId !== variantId) };
    });
  };
  ...
  const handleSelectModel = useCallback((model: GalleryModel) => {
    const profile: ModelProfile = {
      id: model.id, name: model.name, height: model.height, bust: model.bust,
      waist: model.waist, hip: model.hip, skinTone: model.skinTone,
      hairType: model.hairType, hairColor: model.hairColor, generalStyle: model.generalStyle,
    };
